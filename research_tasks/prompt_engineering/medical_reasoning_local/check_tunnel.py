...
The harness expects the SSH tunnel on local port 8111 (reached from the Docker
sandbox at host.docker.internal:8111). But the tunnel's port sometimes changes
between sessions, so if 8111 is down this pokes around the other likely host:port
...
# 8111 is the documented default; the rest are fallbacks the tunnel has used.
PORTS = [8111, 8010, 8000, 8001, 8080, 5000]
...
def main() -> None:
    ...
    if found:
        ...
        print(
            ...
            "8111 -> remote vLLM), then re-run. If it's on a port not listed above, "
